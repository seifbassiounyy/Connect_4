...
def checkDefiniteThree(state: list[list[str]], flag: str) -> int:
    result = 0
    for i in range(6):
        for j in range(3):
            # 0,1,1,1,0
...
def checkTwo(state: list[list[str]]) -> tuple:
    ...
    for i in range(6):
        for j in range(4):
            # 0,0,1,1
            if state[i][j] == '0' and state[i][j + 1] == '0' and state[i][j + 2] != '0' and state[i][j + 2] == state[i][
                j + 3]:
                if state[i][j + 2] == '1':
                    user += 1
                if state[i][j + 2] == '2':
                    agent += 1
            # 1,1,0,0
            if state[i][j] != '0' and state[i][j] == state[i][j + 1] and state[i][j + 2] == '0' and state[i][j + 2] == \
                    state[i][j + 3]:
                if state[i][j] == '1':
                    user += 1
                if state[i][j] == '2':
                    agent += 1
            # 0,1,1,0
            if state[i][j] == '0' and state[i][j + 3] == '0' and state[i][j + 1] != '0' and state[i][j + 1] == state[i][
                j + 2]:
                if state[i][j + 1] == '1':
                    user += 1
                if state[i][j + 1] == '2':
    ...
    for i in range(3):
        for j in range(4):
            ...
            #   1
            # 1
            if state[i][j] != '0' and state[i][j] == state[i + 1][j + 1] and state[i + 3][j + 3] == '0' and \
                    state[i + 4][j + 4] == '0':
                if state[i][j] == '1':
                    user += 1
                if state[i][j] == '2':
            ...
            #       1
            #           1
            if state[i][j] == '0' and state[i + 1][j + 1] == '0' and state[i + 3][j + 3] != '0' and state[i + 3][
                j + 3] == state[i + 4][j + 4]:
                if state[i + 3][j + 3] == '1':
                    user += 1
                if state[i + 3][j + 3] == '2':
